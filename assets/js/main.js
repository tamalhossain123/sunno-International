...
if ($('.tp-header-top-animation').length > 0) {
	...
	/**
 * Service Details Filtering System
 * Ei script-ti sidebar category click korle corresponding content show/hide kore.
 */

document.addEventListener('DOMContentLoaded', function () {
    const categoryItems = document.querySelectorAll('#category-menu li');
    const contentWrappers = document.querySelectorAll('.sv-details-wrapper');

    categoryItems.forEach(item => {
        item.addEventListener('click', function (e) {
            e.preventDefault();

            // 1. Sidebar menu theke active class remove kora ebong click kora item-e add kora
            categoryItems.forEach(li => li.classList.remove('active'));
            this.classList.add('active');

            // 2. data-filter value theke class name ber kora (e.g., ".mer" -> "mer")
            const filterValue = this.getAttribute('data-filter').replace('.', '');

            // 3. Sob content wrapper theke active class remove kora (fole sob gulo hide hoye jabe)
            contentWrappers.forEach(wrapper => {
                wrapper.classList.remove('active');
            });

            // 4. Filter value onujayi specific content div-e active class add kora
            const targetContent = document.querySelector(`.sv-details-wrapper.${filterValue}`);
            if (targetContent) {
